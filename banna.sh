...
# Verifica se l'IP senza .0 è nella lista di esclusione
if grep -q "^$classe_ip_no_dot_zero" /etc/ddos/ignore.ip.list; then
    echo "L'IP $classe_ip_no_dot_zero in lista di esclusione. Nessuna a
zione eseguita."
    exit 0
fi

# Ottieni le informazioni del provider e l'email abuse usando whois
whois_info=$(whois $classe_ip)
provider_name=$(echo "$whois_info" | grep -i 'OrgName\|netname\|descr' | head -n
 1 | awk -F: '{print $2}' | xargs)
abuse_email=$(echo "$whois_info" | grep -i 'abuse\|abuse-mailbox' | head -n 1 | 
awk -F: '{print $2}' | xargs)
